...
func (db *Database) GetAppMetrics(limit int, offset int) ([]AppMetric, error) {
	rows, err := db.db.Query("SELECT id, proto_id, event, value, app_version, operating_system, session_id, created_at, processed FROM app_metrics LIMIT ? OFFSET ?", limit, offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
